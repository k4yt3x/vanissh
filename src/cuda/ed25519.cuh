...
// ---------------------------------------------------------------------------
// SHA-256 of the ssh-ed25519 public key blob (single block): the OpenSSH
...
template <int N>
__device__ __forceinline__ uint32_t rotr32(uint32_t x) {
    return __funnelshift_r(x, x, N);
}

// pk: public key as little-endian words. digest: SHA-256 of the 51-byte blob
// (uint32 11, "ssh-ed25519", uint32 32, key) as big-endian words, i.e.
// digest[0] >> 24 is the first digest byte.
//
// The blob, the 0x80 terminator and the bit length fill exactly one block.
// The rounds are rolled like in SHA-512 above.
__device__ __forceinline__ void sha256_pubkey_blob(const uint32_t pk[8], uint32_t digest[8]) {
    // The key starts at byte 19, so its words straddle the message words.
    uint32_t w[16];
    w[0] = 11;                                    // length of "ssh-ed25519"
    w[1] = 0x7373682du;                           // "ssh-"
    w[2] = 0x65643235u;                           // "ed25"
    w[3] = 0x35313900u;                           // "519", high byte of the key length
    w[4] = 0x00002000u | (bswap32(pk[0]) >> 24);  // key length 32, first key byte
#pragma unroll
    for (int i = 0; i < 7; ++i) {
        w[5 + i] = __funnelshift_l(bswap32(pk[i + 1]), bswap32(pk[i]), 8);
    }
    w[12] = (bswap32(pk[7]) << 8) | 0x80u;  // last three key bytes, terminator
    w[13] = 0;
    w[14] = 0;
    w[15] = 51 * 8;  // message length in bits

    uint32_t a = 0x6a09e667u, b = 0xbb67ae85u, c = 0x3c6ef372u, d = 0xa54ff53au, e = 0x510e527fu,
             f = 0x9b05688cu, g = 0x1f83d9abu, h = 0x5be0cd19u;
...
#pragma unroll
            for (int i = 0; i < 16; ++i) {
                const uint32_t w15 = w[(i + 1) & 15];
                const uint32_t w2 = w[(i + 14) & 15];
                const uint32_t s0 = rotr32<7>(w15) ^ rotr32<18>(w15) ^ (w15 >> 3);
                const uint32_t s1 = rotr32<17>(w2) ^ rotr32<19>(w2) ^ (w2 >> 10);
                w[i] = w[i] + s0 + w[(i + 9) & 15] + s1;
            }
        }
#pragma unroll
        for (int i = 0; i < 16; ++i) {
            const uint32_t big_s1 = rotr32<6>(e) ^ rotr32<11>(e) ^ rotr32<25>(e);
            const uint32_t ch = (e & f) ^ (~e & g);
            const uint32_t t1 = h + big_s1 + ch + kSha256K[16 * block + i] + w[i];
            const uint32_t big_s0 = rotr32<2>(a) ^ rotr32<13>(a) ^ rotr32<22>(a);
